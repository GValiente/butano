...
public:
        ...
        _character_widths(character_widths)
    {
        ...
        BTN_CONSTEXPR_ASSERT(item.tiles().graphics() >= minimum_graphics + int(utf8_characters.size()),
                             "Invalid graphics or utf8 characters count");
        BTN_CONSTEXPR_ASSERT(! item.palette().eight_bits_per_pixel(), "8bpp fonts not supported");
