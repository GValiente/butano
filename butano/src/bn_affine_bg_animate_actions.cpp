...
void iaffine_bg_animate_action::set_next_change_updates(int next_change_updates)
{
...
void iaffine_bg_animate_action::set_update_forever(bool forever)
{
...
void iaffine_bg_animate_action::set_current_index(int current_index, bool update_bg_now)
{
    const ivector<uint16_t>& map_indexes = this->map_indexes();
    ...
    if(update_bg_now)
    {
        _affine_bg_ref->set_map(*_map_item_ref, map_indexes[current_index]);
    }
}

int iaffine_bg_animate_action::current_map_index() const
{
    BN_ASSERT(! done(), "Action is done");
...
void iaffine_bg_cached_animate_action::set_next_change_updates(int next_change_updates)
{
    BN_ASSERT(next_change_updates >= 0 && next_change_updates <= _wait_updates,
              "Invalid next change updates: ", next_change_updates, " - ", _wait_updates);
...
void iaffine_bg_cached_animate_action::set_update_forever(bool forever)
{
    BN_ASSERT(! forever || ! done(), "Action is done");
...
void iaffine_bg_cached_animate_action::set_current_index(int current_index, bool update_bg_now)
{
    const ivector<affine_bg_map_ptr>& maps = this->maps();
    ...
    if(update_bg_now)
    {
        _affine_bg_ref->set_map(maps[current_index]);
    }
}

const affine_bg_map_ptr& iaffine_bg_cached_animate_action::current_map() const
{
    BN_ASSERT(! done(), "Action is done");
