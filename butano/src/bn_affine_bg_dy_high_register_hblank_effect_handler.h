...
#ifndef BN_AFFINE_BG_DY_HIGH_REGISTER_HBLANK_EFFECT_HANDLER_H
#define BN_AFFINE_BG_DY_HIGH_REGISTER_HBLANK_EFFECT_HANDLER_H
...
class affine_bg_dy_high_register_attributes_hblank_effect_handler
{
...
public:
    ...
    static void write_output_values(int, const iany&, const void* input_values_ptr, uint16_t* output_values_ptr)
    {
        ...
        for(int index = 0; index < display::height(); ++index)
        {
            ...
            int result = attributes.dy_register_value() + pd_sum;
...
class affine_bg_dy_high_register_values_hblank_effect_handler
{
