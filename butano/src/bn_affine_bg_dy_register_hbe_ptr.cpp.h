...
affine_bg_dy_register_hbe_ptr affine_bg_dy_register_hbe_ptr::create(
        affine_bg_ptr bg, const span<const affine_bg_mat_attributes>& attributes_ref)
{
    ...
    return affine_bg_dy_register_hbe_ptr(id, true, move(bg));
}
        ...
        affine_bg_ptr bg, const span<const int>& values_ref)
{
    ...
    return affine_bg_dy_register_hbe_ptr(id, false, move(bg));
}

optional<affine_bg_dy_register_hbe_ptr> affine_bg_dy_register_hbe_ptr::create_optional(
        affine_bg_ptr bg, const span<const affine_bg_mat_attributes>& attributes_ref)
{
    ...
    optional<affine_bg_dy_register_hbe_ptr> result;

    if(id >= 0)
    {
        result = affine_bg_dy_register_hbe_ptr(id, true, move(bg));
    }
        ...
        affine_bg_ptr bg, const span<const int>& values_ref)
{
    ...
    if(id >= 0)
    {
        result = affine_bg_dy_register_hbe_ptr(id, false, move(bg));
    }
...
void affine_bg_dy_register_hbe_ptr::reload_attributes_ref()
{
    hblank_effects_manager::reload_values_ref(id());
}

void affine_bg_dy_register_hbe_ptr::reload_values_ref()
{
    hblank_effects_manager::reload_values_ref(id());
}

void affine_bg_dy_register_hbe_ptr::swap(affine_bg_dy_register_hbe_ptr& other)
{
    hbe_ptr::swap(other);
...
affine_bg_dy_register_hbe_ptr::affine_bg_dy_register_hbe_ptr(int id, bool from_attributes, affine_bg_ptr&& bg) :
    hbe_ptr(id),
