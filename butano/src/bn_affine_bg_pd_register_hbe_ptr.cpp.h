...
affine_bg_pd_register_hbe_ptr affine_bg_pd_register_hbe_ptr::create(
        affine_bg_ptr bg, const span<const affine_bg_mat_attributes>& attributes_ref)
{
    ...
    return affine_bg_pd_register_hbe_ptr(id, true, move(bg));
}
        ...
        affine_bg_ptr bg, const span<const int16_t>& values_ref)
{
    ...
    return affine_bg_pd_register_hbe_ptr(id, false, move(bg));
}

optional<affine_bg_pd_register_hbe_ptr> affine_bg_pd_register_hbe_ptr::create_optional(
        affine_bg_ptr bg, const span<const affine_bg_mat_attributes>& attributes_ref)
{
    int id = hblank_effects_manager::create_optional(attributes_ref.data(), attributes_ref.size(), intptr_t(bg.handle()),
                                                     hblank_effects_manager::handler_type::AFFINE_BG_PD_REGISTER_ATTRIBUTES);
    optional<affine_bg_pd_register_hbe_ptr> result;

    if(id >= 0)
    {
        result = affine_bg_pd_register_hbe_ptr(id, true, move(bg));
    }
        ...
        affine_bg_ptr bg, const span<const int16_t>& values_ref)
{
    ...
    if(id >= 0)
    {
        result = affine_bg_pd_register_hbe_ptr(id, false, move(bg));
    }

    return result;
}

span<const affine_bg_mat_attributes> affine_bg_pd_register_hbe_ptr::attributes_ref() const
{
...
span<const int16_t> affine_bg_pd_register_hbe_ptr::values_ref() const
{
...
void affine_bg_pd_register_hbe_ptr::set_attributes_ref(const span<const affine_bg_mat_attributes>& attributes_ref)
{
    BN_BASIC_ASSERT(_from_attributes, "Built from values");
...
void affine_bg_pd_register_hbe_ptr::set_values_ref(const span<const int16_t>& values_ref)
{
    BN_BASIC_ASSERT(! _from_attributes, "Built from attributes");
...
void affine_bg_pd_register_hbe_ptr::reload_attributes_ref()
{
    hblank_effects_manager::reload_values_ref(id());
}

void affine_bg_pd_register_hbe_ptr::reload_values_ref()
{
    hblank_effects_manager::reload_values_ref(id());
}

void affine_bg_pd_register_hbe_ptr::swap(affine_bg_pd_register_hbe_ptr& other)
{
    hbe_ptr::swap(other);
...
affine_bg_pd_register_hbe_ptr::affine_bg_pd_register_hbe_ptr(
