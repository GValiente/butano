...
palette_bitmap_bg_pa_register_hbe_ptr palette_bitmap_bg_pa_register_hbe_ptr::create(
        palette_bitmap_bg_ptr bg, const span<const affine_bg_mat_attributes>& attributes_ref)
{
    ...
    return palette_bitmap_bg_pa_register_hbe_ptr(id, true, move(bg));
}
        ...
        palette_bitmap_bg_ptr bg, const span<const int16_t>& values_ref)
{
    ...
    return palette_bitmap_bg_pa_register_hbe_ptr(id, false, move(bg));
}

optional<palette_bitmap_bg_pa_register_hbe_ptr> palette_bitmap_bg_pa_register_hbe_ptr::create_optional(
        palette_bitmap_bg_ptr bg, const span<const affine_bg_mat_attributes>& attributes_ref)
{
    int id = hblank_effects_manager::create_optional(attributes_ref.data(), attributes_ref.size(), intptr_t(bg.handle()),
                                                     hblank_effects_manager::handler_type::AFFINE_BG_PA_REGISTER_ATTRIBUTES);
    optional<palette_bitmap_bg_pa_register_hbe_ptr> result;

    if(id >= 0)
    {
        result = palette_bitmap_bg_pa_register_hbe_ptr(id, true, move(bg));
    }
        ...
        palette_bitmap_bg_ptr bg, const span<const int16_t>& values_ref)
{
    ...
    if(id >= 0)
    {
        result = palette_bitmap_bg_pa_register_hbe_ptr(id, false, move(bg));
    }

    return result;
}

span<const affine_bg_mat_attributes> palette_bitmap_bg_pa_register_hbe_ptr::attributes_ref() const
{
...
span<const int16_t> palette_bitmap_bg_pa_register_hbe_ptr::values_ref() const
{
...
void palette_bitmap_bg_pa_register_hbe_ptr::set_attributes_ref(
...
void palette_bitmap_bg_pa_register_hbe_ptr::set_values_ref(const span<const int16_t>& values_ref)
{
    BN_BASIC_ASSERT(! _from_attributes, "Built from attributes");
...
void palette_bitmap_bg_pa_register_hbe_ptr::reload_attributes_ref()
{
    hblank_effects_manager::reload_values_ref(id());
}

void palette_bitmap_bg_pa_register_hbe_ptr::reload_values_ref()
{
    hblank_effects_manager::reload_values_ref(id());
}

void palette_bitmap_bg_pa_register_hbe_ptr::swap(palette_bitmap_bg_pa_register_hbe_ptr& other)
{
    hbe_ptr::swap(other);
...
palette_bitmap_bg_pa_register_hbe_ptr::palette_bitmap_bg_pa_register_hbe_ptr(
