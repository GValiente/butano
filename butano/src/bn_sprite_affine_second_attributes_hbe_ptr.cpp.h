...
sprite_affine_second_attributes_hbe_ptr sprite_affine_second_attributes_hbe_ptr::create(
        sprite_ptr sprite, const span<const sprite_affine_second_attributes>& attributes_ref)
{
    ...
    return sprite_affine_second_attributes_hbe_ptr(id, move(sprite));
}

optional<sprite_affine_second_attributes_hbe_ptr> sprite_affine_second_attributes_hbe_ptr::create_optional(
        sprite_ptr sprite, const span<const sprite_affine_second_attributes>& attributes_ref)
{
    int id = hblank_effects_manager::create_optional(attributes_ref.data(), attributes_ref.size(), intptr_t(sprite.handle()),
                                                     hblank_effects_manager::handler_type::SPRITE_AFFINE_SECOND_ATTRIBUTES);
    optional<sprite_affine_second_attributes_hbe_ptr> result;

    if(id >= 0)
    {
        result = sprite_affine_second_attributes_hbe_ptr(id, move(sprite));
    }

    return result;
}

span<const sprite_affine_second_attributes> sprite_affine_second_attributes_hbe_ptr::attributes_ref() const
{
...
void sprite_affine_second_attributes_hbe_ptr::set_attributes_ref(
...
void sprite_affine_second_attributes_hbe_ptr::reload_attributes_ref()
{
    hblank_effects_manager::reload_values_ref(id());
}

void sprite_affine_second_attributes_hbe_ptr::swap(sprite_affine_second_attributes_hbe_ptr& other)
{
    hbe_ptr::swap(other);
    _sprite.swap(other._sprite);
}

sprite_affine_second_attributes_hbe_ptr::sprite_affine_second_attributes_hbe_ptr(int id, sprite_ptr&& sprite) :
    hbe_ptr(id),
    _sprite(move(sprite))
{
}
