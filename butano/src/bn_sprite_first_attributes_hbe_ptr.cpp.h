        ...
        sprite_ptr sprite, const span<const sprite_first_attributes>& attributes_ref)
{
    ...
    if(id >= 0)
    {
        result = sprite_first_attributes_hbe_ptr(id, move(sprite));
    }

    return result;
}

span<const sprite_first_attributes> sprite_first_attributes_hbe_ptr::attributes_ref() const
{
...
void sprite_first_attributes_hbe_ptr::set_attributes_ref(const span<const sprite_first_attributes>& attributes_ref)
{
    hblank_effects_manager::set_values_ref(id(), attributes_ref.data(), attributes_ref.size());
}

void sprite_first_attributes_hbe_ptr::reload_attributes_ref()
{
    hblank_effects_manager::reload_values_ref(id());
}

void sprite_first_attributes_hbe_ptr::swap(sprite_first_attributes_hbe_ptr& other)
{
    hbe_ptr::swap(other);
    _sprite.swap(other._sprite);
}

sprite_first_attributes_hbe_ptr::sprite_first_attributes_hbe_ptr(int id, sprite_ptr&& sprite) :
    hbe_ptr(id),
    _sprite(move(sprite))
{
}
