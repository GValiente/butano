...
enemy::enemy(const enemy_event& event, const bn::sprite_palette_ptr& damage_palette, int8_t tag,
    ...
    _grid_columns(int8_t(bn::max(event.enemy.dimensions.width().shift_integer() / constants::enemies_grid_size, 1))),
    _grid_rows(int8_t(bn::max(event.enemy.dimensions.height().shift_integer() / constants::enemies_grid_size, 1))),
    _last_grid_column(0),
    _last_grid_row(0),
...
void enemy::check_hero_bomb(const bn::point& bomb_center, int bomb_squared_radius, const bn::camera_ptr& camera)
{
    ...
    {
        bn::fixed_point enemy_position = _sprite.position();
        int distance_x = enemy_position.x().shift_integer() - bomb_center.x();
        int distance_y = enemy_position.y().shift_integer() - bomb_center.y();
