...
{
    for(auto bullets_it = _bullets.begin(), bullets_end = _bullets.end(); bullets_it != bullets_end; ++bullets_it)
    {
        btn::fixed_rect bullet_rect(bullets_it->sprite_move_action.sprite().position(),
                                    dimensions[int(bullets_it->type)]);
